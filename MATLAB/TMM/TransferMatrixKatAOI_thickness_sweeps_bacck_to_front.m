...
title('%R for sin^4 profile, n=2 to n=1, 3200 nm thick, 32 layers, 400 nm light');
xlabel('Angle of Incidence (deg)');
ylabel('%R');

% figure
% % mesh(theta_deg,lambda,R)
% contourf(theta_deg,R(:,:,1)'*100)
% title('%R for sin^4 profile, n=2 to n=1, 100 nm thick, 256 layers');
% xlabel('Wavelength (nm)');
% ylabel('Angle of Incidence (deg)');
% zlabel('%R');
