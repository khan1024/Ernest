...
for y = 1:length(total_thickness)
    ...
    for x = 1:number_layers
        layer_thickness = total_thickness(y) / number_layers;
        thicknesses(x+1,y) = layer_thickness;
    end
end
% thickness of each corresponding layer in nm (thickness of the first layer is irrelivant)

gapthickness = 100; %thickness of gap in nm
thicknesses(number_layers+1,:) = gapthickness; 
...
% colorbar
%------------------
contourcreator2(lambda,theta_deg,R'*100)
