...
target_three_quarter_wave = 250;
% The layer thicknesses are in nanometers

total_thickness = 1400:50:1400;

number_layers = 6; % make this one higher than the number of mirror layers you want to include substrate. Air will be included later, too
...
for u = 1:floor(number_layers/2)
    thicknesses(2*u) = target_three_quarter_wave/n_max;
end

for v = 1:floor(number_layers/2)-1
    thicknesses(2*v+1) = target_three_quarter_wave/n_min;
end
...
title({['Reflectance for Dichroic Mirror of ' num2str(number_layers -1) ...
    ' layers, n= ' num2str(n_max) ' and n= ' num2str(n_min)],[...
    ' at ' num2str(round(thicknesses(2,1))) ' and ' num2str(round(thicknesses(3,1))) ...
    ' nm layers and overall thickness of '...
    num2str(round(overall_thickness)) ' nm']},'FontSize',14)
saveas(h1,['.\figures\Reflectance for Dichroic Mirror of ' num2str(number_layers -1) ...
    ' layers, n= ' num2str(floor(n_max)) '_' num2str(100*(n_max - floor(n_max)))...
    ' and n= ' num2str(floor(n_min)) '_' num2str(100*(n_min - floor(n_min)))...
    ' at ' num2str(round(thicknesses(2,1))) ' and ' num2str(round(thicknesses(3,1))) ' nm layers and overall thickness of '...
    num2str(round(overall_thickness)) ' nm.png'])

h2=figure;
contourf(theta_deg,lambda,R(:,:,1)*100,v,'edgecolor','none')
set(gca,'FontSize',14)
colorbar
xlim([0 90])
ylim([400 1300])
...
saveas(h2,['.\figures\Reflectance for Dichroic Mirror of ' num2str(number_layers -1) ...
    ...
    ' and n= ' num2str(floor(n_min)) '_' num2str(100*(n_min - floor(n_min)))...
    ' at ' num2str(round(thicknesses(2,1))) ' and ' num2str(round(thicknesses(3,1))) ' nm layers and overall thickness of '...
    num2str(round(overall_thickness)) ' nm contour.png'])
