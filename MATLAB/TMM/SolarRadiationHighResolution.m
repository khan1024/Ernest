function Results = SolarRadiationHighResolution (Time, Location)
% function Results = SolarRadiationHighResolution(Time, Location)
% 
...
%     Results.DirectRadiation -> direct radiation (W/m)
%     Results.ReflectedRadiation -> reflected radiation (W/m)
%     Results.NormalDirectRadiation -> normalized radiation (W/m)
%     Results.SunAzimuthAngle -> sun's azimuth angle (deg)
...
%     Time.Minutes =0;
%     Time.DayOfYear = 1;
%     Time.StandardMeridian = 1;
%     Time.RunLength = 365*2;
%     Location.Latitude = 0;
...
% 
%     Results = SolarRadiationIL(Time, Location);
%     plot(Resuts.Time, Results.DirectRadiation)  
%
%
...
DayEnd = DayStart + round(Time.RunLength);
...
for intDayOfYear = DayStart:DayEnd
    ...
    loop = 1440*60; % Variable of the "for-loop". The loop repeates only one time
    time = 1;
