...
%
target_three_quarter_wave_min = 230;
target_three_quarter_wave_max = 230;
target_three_quarter_wave_delta = 25;

number_layers_min = 4;
number_layers_max = 16;
number_layers_delta = 2;
...
for target_three_quarter_wave = target_three_quarter_wave_min:target_three_quarter_wave_delta:target_three_quarter_wave_max
    for number_layers = number_layers_min:number_layers_delta:number_layers_max % make this one higher than the number of mirror layers you want to include substrate. Air will be included later, too
        for n_top = n_top_min:n_top_delta:n_top_max
            for n_small = n_small_min:n_small_delta:n_small_max
                for n_large = n_large_min:n_large_delta:n_large_max
                    for n_bottom = n_bottom_min:n_bottom_delta:n_bottom_max

 a = target_three_quarter_wave_min:target_three_quarter_wave_delta:target_three_quarter_wave_max;
 b = number_layers_min:number_layers_delta:number_layers_max;
 c = n_top_min:n_top_delta:n_top_max;
 d = n_small_min:n_small_delta:n_small_max;
 e = n_large_min:n_large_delta:n_large_max;
 f = n_bottom_min:n_bottom_delta:n_bottom_max;
 
 
% error = zeros(length(a)*length(b)*length(c)*length(d)*length(e)*length(f),1);
...
error_increment = error_increment+1;
 for wave = 1:300
     error(error_increment) = sum(R(wave,1,1))/length(lambda);
 end
 for wave = 301:length(lambda)
     error(error_increment) = error(error_increment) + sum(1-R(wave,1,1))/length(lambda);
 end
...
overall_thickness = sum(thicknesses(:,:), 'double');
...
title({['Reflectance for Dichroic Mirror of ' num2str(number_layers -1) ...
    ...
    num2str(round(overall_thickness(1))) ' nm from n=' num2str(n_top) ' to n=' num2str(n_bottom)]},'FontSize',14)
saveas(h1,['.\figures\Reflectance for Dichroic Mirror of ' num2str(number_layers -1) ...
    ...
    num2str(round(overall_thickness(1))) ' nm from n=' num2str(floor(n_top)) '_' num2str(100*(n_top - floor(n_top)))...
...
h3 = figure; 
plot (error)
title ('Error: from 0 for 400-700 and from 1 for 701-1300')
saveas(h3,['.\figures\Deviation from ideal IR mirror for target layer thickness ' num2str(floor(a)) ...
    ', ' num2str(b - 1) ' layers, n_top of ' num2str(floor(c)) '_' num2str(100.*(c - floor(c))) ...
    ', n_bottom of ' num2str(floor(f)) '_' num2str(100.*(f - floor(f))) ', n_small of ' ...
    num2str(floor(d)) '_' num2str(100.*(d - floor(d))) ', and n_large of ' ...
    num2str(floor(e)) '_' num2str(100.*(e - floor(e))) '.png'])
