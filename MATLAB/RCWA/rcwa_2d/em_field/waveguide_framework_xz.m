...
if xz_slice_coordinate_y>coordinate_y_1(end) || xz_slice_coordinate_y<coordinate_y_1(1)
    error('out of range')
end

position_in_eps=1;
for i=2:1:length(coordinate_y_1)
    if coordinate_y_1(i)>=xz_slice_coordinate_y
        break
    else
...
eps_xz_slice=zeros(length(coordinate_x_1)-1,number_of_layers);
...
coord_x=coordinate_x_1;

coordinate_x_1=fliplr(coord_x);

%eps_xz_slice=rot90(eps_xz_slice,3);
%total_thickness=fliplr(total_thickness);
%coordinate_x_1=fliplr(coordinate_x_1);
...
for i=1:1:n_of_rows
    for j=1:1:n_of_columns
        ...
        if matrix_geometry(i,j,1)==2  %nic
            ...
            y_1=coordinate_x_1(i);
            
            x_2=total_thickness(j);
            y_2=coordinate_x_1(i+1);
