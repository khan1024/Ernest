...
if factorization_Li==1
    ...
    %N_xx=asr_fourier_2D(disc_x,disc_y,disc_x,disc_y,abs(N_x_input).^2,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up);
    %N_yy=asr_fourier_2D(disc_x,disc_y,disc_x,disc_y,abs(N_y_input).^2,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up);
    %N_yx=asr_fourier_2D(disc_x,disc_y,disc_x,disc_y,conj(N_y_input).*N_x_input,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up);
    %N_xy=asr_fourier_2D(disc_x,disc_y,disc_x,disc_y,conj(N_x_input).*N_y_input,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up);
end
...
if zeta==0
    if factorization_Li==1
        ...
        G=[-alpha*beta+(mu*k_0^2).*delta_N_xy, alpha*alpha+(mu*k_0^2).*(-E+delta_epsilon*N_yy);
            (mu*k_0^2).*(E-delta_epsilon*N_xx)-beta*beta, alpha*beta-(mu*k_0^2).*delta_N_yx];
