...
pomocna=-Lambda/(d)*(nova_y-d);%inverzní fuknkce

duty_cycle=fliplr((Lambda-(Lambda-pomocna))/Lambda);
layer_thickness=sirka/number_of_layers*ones(1,number_of_layers);%případně zakomentovat
shift=fliplr(pomocna/(2*Lambda));
